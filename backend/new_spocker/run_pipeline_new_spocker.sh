...
# the SPOCKER pipeline) on one PDB. This is what ../serve_pockets.py (in
# Pocket_web_server2/backend/) shells out to; see Script1-8 in this same
...
# Also hand back the raw field grids Script8 actually scored against (same
# trimmed-preferred-over-raw preference as its own RAW_FIELD_SPECS), so
# callers can visualize "where is the stacking field" etc, not just the
# final merged pocket volumes. $FIELDS1_DIR is inside $WORK and would
# otherwise be discarded below.
echo ">>> Collecting scored field grids"
FIELDS_OUT_DIR="$OUT_DIR/fields"
mkdir -p "$FIELDS_OUT_DIR"
_copy_scored_field() {
    local primary="$1" fallback="$2" dest_name="$3"
    if [[ -f "$FIELDS1_DIR/${PDB_ID}${primary}" ]]; then
        cp "$FIELDS1_DIR/${PDB_ID}${primary}" "$FIELDS_OUT_DIR/${dest_name}.mrc"
    elif [[ -n "$fallback" && -f "$FIELDS1_DIR/${PDB_ID}${fallback}" ]]; then
        cp "$FIELDS1_DIR/${PDB_ID}${fallback}" "$FIELDS_OUT_DIR/${dest_name}.mrc"
    fi
}
_copy_scored_field ".apbs_rna_trimmed.mrc" ".apbs.mrc" "apbs"
_copy_scored_field ".hydrophobic_nonoverlap_trimmed.mrc" ".hydrophobic.mrc" "hydrophobic"
_copy_scored_field ".hbacceptors.mrc" "" "hbacceptors"
_copy_scored_field ".hbdonors.mrc" "" "hbdonors"
_copy_scored_field ".stacking.mrc" "" "stacking"
